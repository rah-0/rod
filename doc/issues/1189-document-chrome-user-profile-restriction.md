...
Document this limitation in `NewUserMode`, [the launcher README](../../lib/launcher/README.md), and relevant [user-mode examples](../../lib/examples). Show an explicit persistent automation profile through `UserDataDir`, including that the user must establish a login in that separate profile. Preserve existing user-supplied profile ownership and the installed-browser-only policy.
