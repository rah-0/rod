...
func TestPageHandleFileDialog(t *testing.T) {
	...
	el.MustClick()
	setFiles(repoPath("fixtures/click.html"), repoPath("fixtures/alert.html"))
	...
	}
	{
		...
		el.MustClick()
		g.Err(setFiles([]string{repoPath("fixtures/click.html")}))
...
func TestPageTriggerFavicon(t *testing.T) {
	...
	// test browser in headless mode to trigger favicon request
	{
		...
		s.Route("/test", "")
		s.Route("/favicon.ico", repoPath("fixtures/icon.png"))
		page := g.newPage()
