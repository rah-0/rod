...
//     go test ./tests -run '^TestClick$' -rod=show,devtools
...
const functions = {
  ...
  async overlay(id, left, top, width, height, msg, monospace) {
    await functions.waitLoad()
  ...
  // Returns a waiter for Page.WaitDOMStable. Creating it has no effect on the
  // page. The caller times the stability period, so the wait does not depend
  // on page timers. The start method begins observing the document; start and
  // check return the milliseconds until the period can end, or a status when
  // the wait is over: 'stable' after the document stays stable for ms
  // milliseconds, 'hidden' when the document is unloaded or enters the
  // back/forward cache, 'expired' when a check is overdue, and 'canceled'
  // after the cancel method. Ending the wait disconnects the observer, clears
  // the timer, and removes the event listener.
  //
  // A page that cannot run callbacks, such as one with scripts disabled,
  // drops the observer's records. Digests of the DOM then detect changes
  // between checks.
  waitDOMStable(ms, diff) {
