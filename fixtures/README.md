...
| Files | Purpose |
| --- | --- |
| `click*.html`, `input.html`, `keys.html`, `touch.html`, `double-click.html`, `drag.html`, `native-drag.html`, `mouse-move.html`, `interactable.html` | Pointer, keyboard, form, and interaction behavior. |
...
| `fetch.html` | Network interception. Its `/a` and `/b` endpoints are supplied by [the hijack tests](../tests/hijack_test.go), not files on disk. |
...
Other fixtures remain in their source directories. The [CDP integration tests](../tests/lib/cdp)
read [CDP fixtures](../lib/cdp/fixtures), and the
